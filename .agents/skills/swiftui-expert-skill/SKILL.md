...
- Review accessibility: proper grouping, traits, Dynamic Type support; in this app prefer `.asButton()` for custom tappable views instead of raw tap gestures or ad-hoc `Button { } label: { }` wrappers (see `references/accessibility-patterns.md`)
...
- Improve accessibility: use the project `.asButton()` wrapper for custom tappable views instead of tap gestures or raw `Button { } label: { }`; add `@ScaledMetric` for Dynamic Type (see `references/accessibility-patterns.md`)
...
- Use `.asButton()` for custom tappable elements in this app, add accessibility grouping and labels (see `references/accessibility-patterns.md`)
...
### Accessibility
- Prefer the project `.asButton()` extension over `onTapGesture` or raw `Button { } label: { }` for tappable custom SwiftUI elements. Use native `Button` only where system controls/Form/List semantics are required.
...
// Project tappable glass-style content
Text("Confirm")
    .padding()
